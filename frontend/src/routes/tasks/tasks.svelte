<script>
    // @ts-nocheck
        ...
        const handler = new DataHandler($tasks, {rowsPerPage: 10});
        const rows = handler.getRows();
        
        $: handler.setRows($tasks)
        ...
        async function deleteTask() {
            ...
            tasks.update(prev => prev.filter(m => m.id != id));
        }
    ...
    <div class='segment'>
        ...
        <div class='mobile-grid'>
            {#each $tasks.slice((currentPage - 1) * cardsPerPage, currentPage * cardsPerPage) as task}
                <div class='mobile-card'> 
                    <Link to='/task?id={task.id}' class='itemLink' >{task.name}</Link>
                    <p>{task.make}</p>
                    <p>{task.type}</p>
